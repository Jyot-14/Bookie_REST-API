...
class ErrorHandler {
    ...
    static handleErrors(err, req, res, next) {
        ...
        if (err instanceof typeorm_1.EntityNotFoundError) {
            return Response_1.ResponseUtil.sendError(res, 'Item/page you are looking for does not exist', 404, null);
        }
        if (err.length > 0 && err[0] instanceof class_validator_1.ValidationError) {
            const errors = ErrorHandler.formatErrors(err);
            return Response_1.ResponseUtil.sendError(res, 'Invalid input', 422, errors);
        }
        if (err.message === 'Invalid file type') {
            return Response_1.ResponseUtil.sendError(res, 'Invalid file type', 422, null);
        }
