...
class AuthController {
    ...
    login(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user) {
                return Response_1.ResponseUtil.sendError(res, 'Invalid credentials.', 401, null);
            }
            let passwordMatches = yield (0, bcryptjs_1.compare)(password, user.password);
            if (!passwordMatches) {
                return Response_1.ResponseUtil.sendError(res, 'Invalid credentials.', 401, null);
            }
            let accessToken = (0, jsonwebtoken_1.sign)({ userId: user.id }, process.env.ACCESS_KEY_SECRET || 'secret123', { expiresIn: '30m' });
            const returnUser = user.toResponse();
            return Response_1.ResponseUtil.sendResponse(res, 'User login success', {
                user: returnUser,
                accessToken,
            });
        });
